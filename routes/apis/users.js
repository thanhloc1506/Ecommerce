...
const { createUser, findUserByUserName, findById, verifyPassword } = require('../../services/users');
...
userRouter
	...
	.post('/register/:type', async (req, res) => {
		...
		try {
			findUserByUserName(username)
				.then((foundUser) => {
					if (foundUser) {
						res.status(400).json({ success: false, message: `Username is existing !!` });
						return;
					}
	...
	.post('/login', async (req, res) => {
		...
		try {
			...
			// return res.redirect(`/items/index/${accessToken}`);
		} catch (error) {
